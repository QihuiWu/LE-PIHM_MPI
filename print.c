...
PrintData(FILE ** outp, Control_Data * cD, Model_Data DS, N_Vector CV_Y, realtype t)
{
	...
	if (cD->usDInt >0) {
		avgResults_NV(outp[7], DS->PrintVar[7], CV_Y, cD->usDInt, DS->NumEle, t, 1 * DS->NumEle);
	}
	#endif
	if (cD->surfDInt >0) {
...
PrintData_LE(FILE ** outp_LE, Control_Data * cD, Model_Data DS, N_Vector CV_Y, realtype t)
{
	...
	if (cD->groundelevDInt>0)
		{
			avgResults_NV(outp_LE[0], DS->PrintVar_LE[0], CV_Y, cD->groundelevDInt, DS->NumEle, t, 3 * DS->NumEle);
		}
	if (cD->rockelevDInt>0)
		{
			avgResults_NV(outp_LE[1], DS->PrintVar_LE[1], CV_Y, cD->rockelevDInt, DS->NumEle, t, 4 * DS->NumEle);
		}
	#else
	if (cD->groundelevDInt>0)
		{
			avgResults_NV(outp_LE[0], DS->PrintVar_LE[0], CV_Y, cD->groundelevDInt, DS->NumEle, t, 1 * DS->NumEle);
		}
	if (cD->rockelevDInt>0)
		{
			avgResults_NV(outp_LE[1], DS->PrintVar_LE[1], CV_Y, cD->rockelevDInt, DS->NumEle, t, 2 * DS->NumEle);
		}
	#endif
	if (cD->soilsubDInt>0)
		{
...
PrintData_no_fully_LE(FILE ** outp_LE, Control_Data * cD, Model_Data DS, N_Vector CV_Y, realtype t)
{
	...
	if (cD->groundelevDInt>0)
		{
			avgResults_NV(outp_LE[0], DS->PrintVar_LE[0], CV_Y, cD->groundelevDInt, DS->NumEle, t, 0 * DS->NumEle);
		}
	if (cD->rockelevDInt>0)
		{
			avgResults_NV(outp_LE[1], DS->PrintVar_LE[1], CV_Y, cD->rockelevDInt, DS->NumEle, t, 1 * DS->NumEle);
		}
